...
export const utils = {
  ...
  async redirectBasedOnRole(role) {
    if (role === 'coach') {
        window.location.href = '/coach-admin-panel.html';
    } else if (role === 'student') {
        window.location.href = '/student-panel.html';
    } else {
        // Default redirect if role is unknown or user should not be here
  ...
  async requireAuth(requiredRole = null, skipRedirect = false) {
    ...
    if (userError || !user) {
        if (!skipRedirect) {
            ...
            if (requiredRole) {
                localStorage.setItem('selectedRole', requiredRole);
            }
    ...
    if (profileError || !profile) {
        // This can happen if profile creation failed. Redirect to role page to restart the process.
        if (!skipRedirect && window.location.pathname !== '/role.html' && window.location.pathname !== '/signup.html') {
             alert('Your user profile is not complete. Please select your role to finish setup.');
    ...
    if (requiredRole && profile.role !== requiredRole) {
        if (!skipRedirect) {
            ...
            alert(`Access denied. This page is for ${requiredRole}s, but you are logged in as a ${profile.role}. Redirecting to your dashboard.`);
            await this.redirectBasedOnRole(profile.role);
        }
        // Return user data but indicate role mismatch by returning null profile
        return { user, profile: null }; 
    }
